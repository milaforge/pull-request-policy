...
The policy engine uses a YAML configuration file to define rules. By default, the action reads `.github/pull-request-policy.yml` from the pull request's base SHA, not from the PR head. A policy change therefore takes effect only after it merges. See [Base-SHA Configuration Loading](base-sha-config.md) for the security model and implications for self-tests.
